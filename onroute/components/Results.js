...
const Results = ({ route, tripData, onBackToForm }) => {
  ...
  const handleRestaurantSelect = (restaurantInfo) => {
    ...
    const alreadySelected = selectedRestaurants.some(
      (r) => r.name === restaurantInfo.name
    );

    if (alreadySelected) {
      // Remove from selection
      setSelectedRestaurants((prev) =>
        prev.filter((r) => r.name !== restaurantInfo.name)
      );
    } else {
      // Add to selection
  ...
  const parseRestaurantSuggestions = async () => {
    ...
    try {
          ...
          if (nameMatch) {
            ...
            const restaurant = {
              ...
              location: addressMatch
                ? await getCoordinatesFromAddress(addressMatch[1].trim())
                : { lat: 0, lng: 0 },
        ...
        if (suggestions.places) {
          ...
          for (const place of suggestions.places) {
            ...
            placesWithCoordinates.push({
              ...
              type: "restaurant",
              description:
                place.description || `${place.name} - ${place.address || ""}`,
            });
          }
      ...
      console.error(
        "Could not parse restaurant suggestions format:",
        suggestions
      );
      return [];
    } catch (error) {
  ...
  const showSuggestionSelector =
    tripData?.useCustomPrompt && parsedSuggestions.length > 0;

  return (
    <div className="py-8 px-4 max-w-6xl mx-auto">
      <h2 className="text-3xl font-bold text-center mb-6">
        Your Recommended Stops
      </h2>
      <button
      ...
      <div className="mb-6">
        ...
        {tripData?.useCustomPrompt && (
          <p>Your Prompt: "{tripData.customPrompt}"</p>
        )}
      </div>
        ...
        <div className="mb-6 p-4 border rounded-lg bg-gray-50">
          ...
          {parsedSuggestions.length === 0 && (
            <p className="text-gray-500">
              No restaurant suggestions were found for your query.
            </p>
          )}
          ...
          >
            Add {selectedRestaurants.length} Restaurant
            {selectedRestaurants.length !== 1 ? "s" : ""} to Route
          </button>
        </div>
      )}
