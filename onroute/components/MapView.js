...
export default function MapView({ stops }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const destStop =
        stops.find((stop) => stop.type === "destination") ||
              ...
              if (status === "MAX_WAYPOINTS_EXCEEDED" && waypoints.length > 0) {
                directionsServiceRef.current.route(
                  ...
                  (simpleResult, simpleStatus) => {
                    if (
                      simpleStatus === window.google.maps.DirectionsStatus.OK
                    ) {
                      setDirections(simpleResult);
                      setMapError(
                        "Simplified route shown - some stops omitted"
                      );
                    } else {
        ...
        <div className="border border-gray-300 w-full h-64 rounded-lg flex items-center justify-center bg-gray-100">
          <div className="text-center p-4">
            ...
            <p className="text-gray-500 text-sm">
              From: {stops[0]?.name || "Start location"}
            </p>
            <p className="text-gray-500 text-sm">
              To: {stops[stops.length - 1]?.name || "Destination"}
            </p>
          </div>
        </div>
        <a
          href={`https://www.google.com/maps/dir/${encodeURIComponent(
            stops[0]?.name || ""
          )}/${encodeURIComponent(stops[stops.length - 1]?.name || "")}/`}
          target="_blank"
