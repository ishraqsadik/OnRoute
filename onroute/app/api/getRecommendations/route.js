...
export async function POST(request) {
  try {
    ...
    try {
      ...
      console.log(
        `Calling Llama API endpoint: ${llamaEndpoint} with data:`,
        llamaRequestData
      );
      ...
      if (llamaResponse.data && llamaResponse.data.status === "success") {
        ...
        const transformedResponse = transformLlamaResponse(
          llamaData,
          startCoords,
          destCoords,
          start,
      ...
      // Create Google Maps directions link with consistent format
...
function transformLlamaResponse(
  llamaData,
  startCoords,
  destCoords,
  startName,
  ...
  // Generate Google Maps directions link with waypoints
  // Extract all stops except start and destination to use as waypoints
  const waypointStops = stops.slice(1, -1);

  let googleMapsLink;
  if (waypointStops.length > 0) {
    // Create waypoints string for Google Maps URL
    const waypointsString = waypointStops
      .map((stop) => {
        // If we have a specific address, use it
        if (stop.address) {
          return `via:${encodeURIComponent(stop.address)}`;
        }
    ...
    googleMapsLink = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(
      ...
      destName
    )}&waypoints=${waypointsString}&travelmode=driving`;
  } else {
    // If no waypoints, use simple URL
    googleMapsLink = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(
      startName
