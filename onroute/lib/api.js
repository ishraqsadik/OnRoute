...
const isDevelopmentMode = () => {
  return (
    ...
    (typeof window !== "undefined" &&
      localStorage.getItem("dev_mode_forced") === "true")
  );
};
...
export const getRecommendations = async (tripData) => {
  try {
    ...
    console.log(
      `Using API endpoint: ${endpoint}, authenticated: ${isLoggedIn}`
    );

    // Geocode the locations before sending to API
    const geoData = await geocodeLocations(
      tripData.start,
    ...
    if (process.env.NEXT_PUBLIC_USE_LLAMA_MODEL === "true") {
      try {
        ...
        console.log(
          `Calling Llama model API endpoint: ${llamaEndpoint} with data:`,
          llamaRequestData
        );
        const llamaResponse = await llamaApi.post(
          llamaEndpoint,
          llamaRequestData
        );
        ...
        if (llamaResponse.data && llamaResponse.data.status === "success") {
          console.log(
            "Received successful response from Llama model:",
            llamaResponse.data
          );
          ...
          console.error(
            "Invalid response from Llama model:",
            llamaResponse.data
          );
...
function transformLlamaResponse(llamaData, tripData, geoData) {
  ...
  if (tripData.useCustomPrompt) {
    ...
    if (llamaData.travel_plan || llamaData.restaurant_suggestions) {
      ...
      if (
        llamaData.travel_plan &&
        llamaData.travel_plan.stops_with_restaurants
      ) {
        for (const stopData of llamaData.travel_plan.stops_with_restaurants) {
          ...
          if (stopData.places && stopData.places.length > 0) {
            ...
            const topPlace = [...stopData.places].sort(
            ...
            if (topPlace) {
              stops.push({
                location: {
                  lat:
                    topPlace.location?.lat ||
                    topPlace.coordinates?.latitude ||
                    0,
                  lng:
                    topPlace.location?.lng ||
                    topPlace.coordinates?.longitude ||
                    0,
                },
  ...
  if (
    llamaData.stops_with_restaurants ||
    (llamaData.travel_plan && llamaData.travel_plan.stops_with_restaurants)
  ) {
    const stopsData =
      llamaData.stops_with_restaurants ||
      llamaData.travel_plan.stops_with_restaurants;
      ...
      if (stopData.places && stopData.places.length > 0) {
        ...
        if (topPlace) {
          stops.push({
            location: {
              lat:
                topPlace.location?.lat || topPlace.coordinates?.latitude || 0,
              lng:
                topPlace.location?.lng || topPlace.coordinates?.longitude || 0,
            },
            type: stopInfo.type.toLowerCase(),
            name: topPlace.name,
            address: topPlace.address || topPlace.vicinity,
  ...
  return {
    route: {
      ...
      summary:
        llamaData.route_summary ||
        (llamaData.travel_plan && llamaData.travel_plan.route_summary),
    },
  };
}
...
const geocodeLocations = async (start, destination) => {
  ...
  if (!window.google || !window.google.maps || !window.google.maps.Geocoder) {
    throw new Error(
      "Google Maps Geocoder not available. Please check your API key and connection."
    );
  }
  ...
  try {
    ...
    throw new Error(
      "Failed to geocode locations. Please check your addresses and try again."
    );
  }
};
